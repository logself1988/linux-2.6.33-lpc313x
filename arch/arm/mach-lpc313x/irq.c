...
#define NUM_IRQ_EVENT (sizeof(irq_2_event)/sizeof(irq_2_event[0]))
...
{
	u32 evt = EVENT_FOR_IRQ(irq);
	u32 bank = EVT_GET_BANK(evt);
	u32 bit_pos = evt & 0x1F;

	EVRT_MASK_CLR(bank) = _BIT(bit_pos);
	EVRT_OUT_MASK_CLR(0, bank) = _BIT(bit_pos);
}
...
#define ROUTER_HDLR(n) \
	static void router##n##_handler (unsigned int irq, struct irq_desc *desc) { \
		...
		for(irq = IRQ_EVT_START; irq < IRQ_EVT_END; irq++) { \
			evt = EVENT_FOR_IRQ(irq); \
...
void __init lpc313x_init_irq(void)
{
	...
	/* Configure all evtr interrupts */
	for (irq = IRQ_EVT_START; irq < IRQ_EVT_END; irq++) {
		int evt = EVENT_FOR_IRQ(irq);
		bank = EVT_GET_BANK(evt);
		bit_pos = evt & 0x1F;
	...
	/* Now configure legacy event interrupts */
	for (i = 0; i < NUM_IRQ_EVENT; i++) {
		...
		printk("legacy evt irq %d event:0x%x bank:%d bit:%d type:%d\n",
			irq,
